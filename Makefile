...
RELEASE_FILES = LICENSE README.md CHANGELOG.md app-sample.yaml docs
...
BUILD_VERSION := $(shell git describe --tags --always | cut -f 1 -d "-")
BUILD_DATE := $(shell date +'%Y-%m-%d %H:%M:%S')
SHA_SHORT := $(shell git rev-parse --short HEAD)

TAGS = ""
MOD_NAME = github.com/alimy/navis
...
build:
	@echo Build navis
	@go build -trimpath -tags '$(TAGS)' -ldflags '$(LDFLAGS)' -o $(RELEASE_ROOT)/$(TARGET) $(TARGET_DIR) 

run:
	@go run -trimpath -gcflags "all=-N -l" -tags '$(TAGS)' -ldflags '$(LDFLAGS)' $(TARGET_DIR) serv
...
.PHONY: linux-amd64
linux-amd64:
	@echo Build navis[linux-amd64] CGO_ENABLED=$(CGO_ENABLED)
	@CGO_ENABLED=$(CGO_ENABLED) GOOS=linux GOARCH=amd64 go build -trimpath -tags '$(TAGS)' -ldflags '$(LDFLAGS)' -o $(RELEASE_LINUX_AMD64)/$(TARGET_BIN) $(TARGET_DIR)

.PHONY: darwin-amd64
darwin-amd64:
	@echo Build navis[darwin-amd64] CGO_ENABLED=$(CGO_ENABLED)
	@CGO_ENABLED=$(CGO_ENABLED) GOOS=darwin GOARCH=amd64 go build -trimpath  -tags '$(TAGS)' -ldflags '$(LDFLAGS)' -o $(RELEASE_DARWIN_AMD64)/$(TARGET_BIN) $(TARGET_DIR)

.PHONY: darwin-arm64
darwin-arm64:
	@echo Build navis[darwin-arm64] CGO_ENABLED=$(CGO_ENABLED)
	@CGO_ENABLED=$(CGO_ENABLED) GOOS=darwin GOARCH=arm64 go build -trimpath -tags '$(TAGS)' -ldflags '$(LDFLAGS)' -o $(RELEASE_DARWIN_ARM64)/$(TARGET_BIN) $(TARGET_DIR)

.PHONY: windows-x64
windows-x64:
	@echo Build navis[windows-x64] CGO_ENABLED=$(CGO_ENABLED)
	@CGO_ENABLED=$(CGO_ENABLED) GOOS=windows GOARCH=amd64 go build -trimpath  -tags '$(TAGS)' -ldflags '$(LDFLAGS)' -o $(RELEASE_WINDOWS_AMD64)/$(TARGET_BIN).exe $(TARGET_DIR)
